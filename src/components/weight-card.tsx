...
export function WeightCard({
  ...
  const delta =
    current && start !== null
      ? Number(current.weight_kg) - Number(start)
      : null;
  ...
  return (
    <section className="border-line bg-surface mt-6 rounded-2xl border p-3 sm:p-4">
      <button
        ...
        aria-expanded={open}
        aria-controls="peso-detalhes"
        className="flex min-h-11 w-full items-center gap-2 text-left"
      >
        <Scale aria-hidden className="text-muted h-4 w-4 shrink-0" />
        <span className="text-ink flex-1 text-lg leading-tight font-semibold tracking-tight">
          Peso
        </span>
        <ChevronDown
          aria-hidden
      ...
      {/* Os três números ficam fora do colapso: são eles o valor de relance,
          e o que custa altura é o gráfico. */}
      ...
      >
        <div id="peso-detalhes" className="overflow-hidden" inert={!open}>
          <form onSubmit={handleSubmit} className="mt-4 flex gap-2">
            <input
              ...
              placeholder={
                thisWeek
                  ? `${Number(thisWeek.weight_kg).toFixed(1)}`
                  : "ex: 74.5"
              }
